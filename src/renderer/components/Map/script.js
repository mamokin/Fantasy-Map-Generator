...
.on('click', editRiver)
  ...
  function editIcon() {
    ...
    if (elSelected) {
      if (this.isSameNode(elSelected.node())) {return;}
    }
...
.attr('height', size)
  ...
  function calculatePopulation() {
    ...
    states.map((s, i) => {
      ...
      const regionCells = $.grep(cells, (e) => {
        return e.region === i;
      });
      let cellsPop = 0;
      regionCells.map((c) => {
  ...
  function checkAccessibility() {
    ...
    for (let f = 0; f < features.length; f++) {
      ...
      if (lakePorts.length) {
        ...
        lakePorts.forEach((p) => {
          if (lakes[p.port] === 1) {p.port = undefined;}
        });
      }
  ...
  function generateStateName(state) {
    let culture = null;
    if (states[state]) {
      if (manors[states[state].capital]) {culture = manors[states[state].capital].culture;}
    }
  ...
  function getCultureId(c) {
    for (let i = 0; i < cultures.length; i++) {
      if (cultures[i].center[0] === c[0]) {
        if (cultures[i].center[1] === c[1]) {return i;}
      }
    }
  }
    ...
    function addRiverOnClick() {
      ...
      while (cell) {
        ...
        if (cells[min].height < 20) {
          ...
          const riverCells = $.grep(land, (e) => {
            return e.river === r;
          });
          riverCells.sort((a, b) => {
            return b.height - a.height
          });
          const riverCellsUpper = $.grep(riverCells, (e) => {
